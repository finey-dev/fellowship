...
class BibleRepository {
  ...
  final _mapOfBibleBooksCodes = <int, String>{
    1: 'GEN',
    2: 'EXO',
    3: 'LEV',
    4: 'NUM',
  ...
  Map<int, String> get mapOfBibleBookCodes => _mapOfBibleBooksCodes;

  List<Book> getBooks() {
    var books = <Book>[];

    // Combine the two maps
    var combinedMap = Map.fromEntries(
      _mapOfBibleBooks.entries.map((item) {
        var bookCode = _mapOfBibleBooksCodes[item.key];
        return MapEntry(item.key, {'name': item.value, 'bookCode': bookCode,});
      }),
    );

    for (var item in combinedMap.entries.toList()) {
